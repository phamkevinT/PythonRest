...
def verifyPw(username, password):
    """
    Verify that password matches that of corresponding username in database
    """
...
def countTokens(username):
    """
    Get the number of token a user has
    """
    tokens = users.find({
        "Username": username
    })[0]["Tokens"]
...
class Detect(Resource):
    def post(self):
        ...
        # Calculate similarity between text1, text2

        nlp = spacy.load('en_core_web_sm')
        text1 = nlp(text1)
        text2 = nlp(text2)

        # Ratio is a number between 0 and 1 with 1 being hightest similarity
